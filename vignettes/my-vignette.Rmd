...
r <- reslifefsr(data = fs1, life = 1, type = 'mean')
r
```
...
```{r}
head(reslifefsr(data = fsr2, life= 4), 10)
```
If we want to look at the median, we can specify that.

```{r}
head(reslifefsr(data = fsr2,life= 4, type = 'median'),10)
```
We can look at all 3 types now (mean, median, and percentile).

```{r}
head(reslifefsr(data = fsr2, life= 4, p = .8, type = 'all'),10)
```
...
```{r}
reslifefsr(data = fsr2,life= 4,p = .6, type = 'all', newdata= newdata)
```
...
reslifefsr(data = fsr2,life= 4,p = .6, type = 'all', newdata= newdata2)
```
...
reslifefsr(data = fsr2, life = 4,p = .6, type = 'all', newdata= newdata3)
```
...
within flexsurv (gamma, gengamma.orig, weibull, gompertz, exponential, lnorm, llogis, gengamma), and the names of the parameters must match those specified for each distribution. 

We will show a demonstration below. 

The inputs for the function are simple. The minimum and maximum values for the are the first two arguments,
1 and 20. The next argument is the distribution name, 'weibull'. The last argument are the parameters, 
inputted as a vector. The names, shape and scale, are specified in the vector. 
```{r}
residlife(min = 1,max =  20, distribution = 'weibull', parameters= c(shape= .9, scale = 2.2))
```

What if the parameter names are incorrect? The function will return an error. 

```{r}
residlife(min = 1, max= 20, distribution= 'weibull', parameters= c(shape= .9, not_scale = 2.2))
```

For distributions where there are different parameterizations, the function can be flexible.

For example, gamma has both shape/rate and shape/scale parameterizations. he function works for 
both, as long as the parameters are correctly specified in the input. 

```{r}
residlife(min = 1, max= 20, distribution= 'gamma', parameters= c(shape= .9, scale = 2.2))
```

```{r}
residlife(min = 1, max = 20, distribution= 'gamma', parameters= c(shape= .9, rate = 1/2.2))
```

Much like the 'reslife.fsr' function, the 'reslife' function has the ability to show mean, median,
percentile, or all three. 

```{r}
residlife(min = 1, max= 20, distribution= 'weibull', parameters= c(shape= .9, scale = 2.2), p = .7, type = 'all')
```
