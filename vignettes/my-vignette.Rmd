...
```{r}
residlife(values = seq(1,20), distribution = 'weibull', parameters= c(shape= .9, scale = 2.2))
```

What if the parameter names are incorrect? The function will return an error. 

```{r}
residlife(values = seq(1,20), distribution= 'weibull', parameters= c(shape= .9, not_scale = 2.2))
```
...
```{r}
residlife(values = seq(1,20), distribution= 'gamma', parameters= c(shape= .9, scale = 2.2))
```

```{r}
residlife(values = seq(1,20), distribution= 'gamma', parameters= c(shape= .9, rate = 1/2.2))
```
...
```{r}
residlife(values = seq(1,20), distribution= 'weibull', parameters= c(shape= .9, scale = 2.2), p = .7, type = 'all')
```
