...
reslife.fsr(fsr2, 4,p = .6, type = 'all', newdata= newdata3)
```
...
```{r}
residlife(1, 20, 'weibull', c(shape= .9, scale = 2.2))
```

What if the parameter names are incorrect? The function will return an error. 

```{r}
residlife(1, 20, 'weibull', c(shape= .9, not_scale = 2.2))
```
...
```{r}
residlife(1, 20, 'gamma', c(shape= .9, scale = 2.2))
```

```{r}
residlife(1, 20, 'gamma', c(shape= .9, rate = 1/2.2))
```
...
```{r}
residlife(1, 20, 'weibull', c(shape= .9, scale = 2.2), p = .7, type = 'all')
```
